// DYNOTEST TANPA MEMPERHITUNGKAN BERAT KENDARAAN DAN BERAT PENGEMUDI
// DYNOTEST DENGAN 2 SENSOR YAKNI SENSOR ENCODER ROLLER DAN RPM ENGINE
...
float inertiaRoller; // Momen inersia roller
...
void setup() {
  ...
  inertiaRoller = 0.5 * rollerMass * pow(rollerRadius, 2);
}
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    // Menghitung energi rotasi
    float energyRoller = 0.5 * inertiaRoller * pow(angularVelocityRoller, 2);

    // Menghitung delta energi rotasi
    deltaEnergy = energyRoller - (0.5 * inertiaRoller * pow(previousAngularVelocityRoller, 2));
